...
Result<Observation<std::vector<SocketInfo>>> NetlinkSocketProvider::query(const SocketQuery& query) const {
 auto opened=NetlinkSocket::open();if(!opened)return Result<Observation<std::vector<SocketInfo>>>::failure(opened.error());auto socket=std::move(opened).value();Observation<std::vector<SocketInfo>> observed{{},{}};std::optional<Error> last;std::uint32_t seq=1;
 for(auto protocol:{TransportProtocol::Tcp,TransportProtocol::Udp}){if(query.protocol&&*query.protocol!=protocol)continue;for(auto family:{AddressFamily::IPv4,AddressFamily::IPv6}){auto result=one(socket,family,protocol,seq++);if(!result){last=result.error();observed.warnings.push_back(warning(result.error()));continue;}for(auto& item:result.value()){if(protocol==TransportProtocol::Udp&&query.listeningOnly&&item.remote)continue;if(query.localPort&&item.local.port!=*query.localPort)continue;observed.value.push_back(std::move(item));}}}
 if(observed.value.empty()&&last&&observed.warnings.size()>=4)return Result<Observation<std::vector<SocketInfo>>>::failure(std::move(*last));
